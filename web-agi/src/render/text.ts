 ...
 *
 * The border is drawn in the text colour a pixel outside the text block, which
 * is why this reaches for pixels rather than staying in cells.
 */
export function drawWindow(display: Display, window: TextWindow): void {
  ...
  const left = window.column * CHAR_WIDTH;
  const top = window.row * CHAR_HEIGHT;
  const width = textWidth * CHAR_WIDTH;
  const height = window.lines.length * CHAR_HEIGHT;

  // Border first, then the text area inside it.
  display.fillRect(left - 3, top - 3, width + 6, height + 6, window.foreground);
  display.fillRect(left - 1, top - 1, width + 2, height + 2, window.background);
